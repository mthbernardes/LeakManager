...
function searchbyUsername() {
    var infos = {};
    $('#leaksresultUsername').html("")
    infos['search'] = document.getElementsByName('username-search')[0].value
    infos['searchby'] = 'username'
    $.ajax({
        url: '/search',
        type: 'POST',
        data: JSON.stringify(infos),
        ...
        success: function(data) {
            data.forEach(function(leak, index) {
                $('#leaksresultUsername').append('<tr><td>' + escape(leak.email) + '</td><td>' + escape(leak.username) + '</td><td>' + escape(leak.password) + '</td><td>' + escape(leak.database) + '</td><td><a href="/delete/' + escape(leak.id) + '">delete</a></td></tr>')
            });
        }
    });
}
